...
func shouldRetryTest(ctx context.Context, test *testCase, permittedRetryImageTags []string) bool {
	...
	retryTestNames := []string{
		...
		"[sig-node] Probing container should *not* be restarted with a non-local redirect http liveness probe [Suite:openshift/conformance/parallel] [Suite:k8s]",                                              // https://issues.redhat.com/browse/OCPBUGS-59735
		"[sig-node] [Feature:SidecarContainers] Probing restartable init container should *not* be restarted with a non-local redirect http liveness probe [Suite:openshift/conformance/parallel] [Suite:k8s]", // https://issues.redhat.com/browse/OCPBUGS-60643
	}
